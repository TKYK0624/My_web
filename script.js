...
document.addEventListener("DOMContentLoaded", () => {
	...
	const hoverTargets = document.querySelectorAll(
		"a, .nav-button, .portfolio-card, .portrait"
	);
...
//影片播放
document.addEventListener("DOMContentLoaded", () => {
	const videoPlayer = document.getElementById("gameVideo");
	...
	const playNextVideo = () => {
		...
		videoPlayer.load();
		videoPlayer.play().catch((e) => console.log("自動播放被攔截或載入失敗:", e));
	};

	// 正常播放結束後跳下一個
	videoPlayer.addEventListener("ended", playNextVideo);

	// 如果影片載入失敗（例如路徑寫錯），也跳下一個
	videoPlayer.addEventListener("error", () => {
		console.error(
			"影片載入失敗，路徑可能錯誤：" + videoSources[currentVideoIndex]
		);
		playNextVideo();
	});
});